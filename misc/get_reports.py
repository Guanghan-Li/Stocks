...
def createThreadFunc(thread_name):
    def threadFunc(queue, asset_group, broker, now):
        # date_format = "%Y-%m-%d"
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        # now = datetime.now().strftime(date_format)
        broker.getPriceData(asset_group, queue.sync_q, thread_name, end_date=now)
...
def prepareData(prices):
    price_data = []
    for price in prices:
        new_price = {"h": price["high"], "l": price["low"]}
...
async def getData(now):
    ...
    for i in range(10):
        ...
        thread = threading.Thread(
            target=threadFunc, args=[queue, amount[i], broker, now]
        )
    ...
    for i in range(asset_amount):
        ...
        now = datetime.fromisoformat("2022-02-16")
        # now = datetime.now()
...
async def joined(session: Session, details):
    ...
    start = end - relativedelta(days=60)
