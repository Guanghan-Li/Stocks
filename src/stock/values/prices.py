...
class Prices:
    ...
    def splitByYear(self) -> list["Prices"]:
        ...
        return [
            Prices(self.symbol, list(group)[::-1])
            for group in groups
            if len(group) >= 252
        ][::-1]
    ...
    def getBefore(self, date: datetime) -> "Prices":
        ...
        new_prices = [
            price for price in self.prices if self.makeDateGood(price.date) <= date
        ]
    ...
    def splitAt(self, date: datetime):
        date = self.makeDateGood(date)
        before_prices: list[Price] = [
            price for price in self.prices if self.makeDateGood(price.date) <= date
        ]
        after_prices: list[Price] = [
            price for price in self.prices if self.makeDateGood(price.date) > date
        ]
    ...
    def adjust(self, announcement: Announcement):
        date = datetime(
            ...
            announcement.ex_date.month,
            announcement.ex_date.day,
            tzinfo=pytz.UTC,
        )
    ...
    def stockSplit(self, new_rate, old_rate, date):
        ...
        adjusted_prices = [
            price.stockSplit(new_rate, old_rate) for price in before_prices
        ]
    ...
    def stockReverseSplit(self, new_rate, old_rate, date):
        before_prices, after_prices = self.splitAt(date)
        adjusted_prices = [
            price.stockReverseSplit(new_rate, old_rate) for price in before_prices
        ]
        return adjusted_prices + after_prices
