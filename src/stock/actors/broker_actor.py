...
class BrokerActor(ActorTypeDispatcher):
    ...
    def receiveMsg_GetPriceMessage(self, message: GetPriceMessage, sender):
        ...
        for asset in asset_group:
            ...
            prices: Prices = self.broker.getPriceData(
                asset, start_date=start_date, end_date=end_date, thread_name=self.name
            )
            ...
            if (not prices.empty) and prices.amountOfYears() >= 4:
                ...
                self.log.info(
                    f"{fg.green}END getPrices{fg.rs} {self.name} | asset: {asset} | took: {time_spent}"
                )
            else:
                end = datetime.now()
                time_spent = (end - start).microseconds * 0.001
                self.log.info(
                    f"{fg.green}END getPrices nothing sent {prices.symbol} {fg.rs} {self.name} | amount {len(prices)} | years: {prices.amountOfYears()} | took: {time_spent}"
                )
