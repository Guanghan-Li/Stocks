...
class Broker:
    ...
    def getPriceData(
        self, asset, start_date: datetime, end_date: datetime, thread_name=""
    ):
        self.log.info("Getting Price data for", asset)
        start_date = start_date.strftime("%Y-%m-%d")
        end_date = end_date.strftime("%Y-%m-%d")

        whole_data = self.api.get_bars(
            asset, TimeFrame.Day, start_date, end_date, adjustment="raw"
        )
    ...
    async def getPrices(self, asset, start_date: datetime, end_date: datetime):
        ...
        asset, whole_data = await self.async_api.get_bars_async(
            asset, TimeFrame.Day, start_date, end_date, adjustment="raw"
        )
    ...
    def getOrders(self, report, buying_power):
        ...
        sum_momentum = sum(
            [round(abs(stock.current_momentum), 3) * 1000 for stock in stocks]
        )
        ...
        for stock in stocks:
            percentage = round(
                (round(stock.current_momentum, 3) * 1000) / sum_momentum, 4
            )
