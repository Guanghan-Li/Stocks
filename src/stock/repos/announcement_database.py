...
class AnnouncementDatabase:
    ...
    def saveAnnouncement(self, announcement: Announcement) -> Announcement:
        ...
        with self.database.atomic():
            if (
                AnnouncementModel.select()
                .where(AnnouncementModel.id == announcement.id)
                .exists()
            ):
    ...
    def listAnnouncements(self, symbol=None):
        if symbol == None:
            ...
            dbs = list(
                AnnouncementModel.select().where(
                    (AnnouncementModel.target_symbol == symbol)
                    | (AnnouncementModel.initiating_symbol == symbol)
                )
            )
    ...
    def exist(self, target_symbol):
        query = AnnouncementModel.select().where(
            AnnouncementModel.target_symbol == target_symbol
        )
        return query.exists()
