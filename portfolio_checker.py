            ...
            for cutoff in list(Cutoff):
                for secondary_sorting in list(Sorting):
                    for portfolio_size in list(PortfolioSize):
                        strategy = Strategy(
                            [filter, Filter.TREND_UP, Filter.COLUMN_UP],
                            initial_sorting,
                            cutoff,
                            secondary_sorting,
                            portfolio_size,
...
async def main():
    ...
    exclude_stocks = [
        announcement.initiating_symbol
        for announcement in announcements
        if announcement.target_symbol != ""
    ]
    exclude_stocks += [
        announcement.target_symbol
        for announcement in announcements
        if announcement.target_symbol != ""
        and not announcement.target_symbol in exclude_stocks
    ]
...
async def checkStrategy(date, strategy, report):
    ...
    for stock in stocks:
        data = api.get_bars(
            stock, TimeFrame.Day, "2022-05-25", "2022-05-26", adjustment="raw"
        )
