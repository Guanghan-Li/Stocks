...
class BrokerActor(ThreadingActor):
  def __init__(self, name, save_price_actor):
    super().__init__()
    self.name = name
    self.save_price_actor = save_price_actor
    #self.gen_report_actor = gen_report_actor
  
  def getPrices(self, asset_group, broker):
    start_date = datetime(2018, 2, 21)
    end_date = datetime(2022, 2, 23)
    for asset
    prices = broker.getPriceData(asset_group, start_date=start_date, end_date=end_date, thread_name=self.name)
    save_price_actor.savePrice(prices)
    return prices
...
class SavePriceActor(ThreadingActor):
  ...
  def savePrice(self, message):
    print(f"{self.name} getting data now...")
    asset = message["asset"]
    data = message["data"]
    print(f"{self.name} GOT data for {asset}")
    self.prices_database.setupPrices(asset, data)
    print(f"{self.name} done saving for {asset}")
...
async def main():
  for i in range(10):
    broker_actor = BrokerActor.start(amount[0], brokers[i//2]).proxy()
    save_price = SavePriceActor.start().proxy()
