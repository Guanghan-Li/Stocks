...
strategy = Strategy(Filter.ACCELERATION_MIN, Sorting.RSI14_DOWN, 50, Sorting.TWO_YEAR_MOMENTUM_DOWN, PortfolioSize.FOUR)
...
async def main():
  ...
  report = report_database.getReports(now, strategy)
  original_order_stocks = [entry.stock for entry in report.entries]
  report.entries = sorted(report.entries, key=lambda entry: entry.current_momentum, reverse=True)
  report.entries = report.entries[:4]
