...
async def main():
  ...
  now = datetime.fromisoformat("2021-10-10")
  report = report_database.getReports(now, 500)
  original_order_stocks = [entry.stock for entry in report.entries]
  report.entries = sorted(report.entries, key=lambda entry: entry.acceleration, reverse=True)
  report.entries = report.entries[:5]
  stocks = [entry.stock for entry in report.entries]
  queue = janus.Queue()
  price_data = {}
  for stock in stocks:
    data = api.get_bars(stock, TimeFrame.Day, "2021-11-29", "2021-11-30", adjustment='raw')
