...
def getTwoYearPrices(stock, prices: DataFrame, orig_date: datetime):
  ...
  print("BYE5")
  prices = prices.loc[str(start):str(orig_date)]
  print("BYE6")
...
async def main():
  ...
  for i in range(1):
    ...
    broker = brokers[i//2]
    thread = threading.Thread(target=threadFunc, args=[price_queue, gen_report_queue, amount[i][:1], broker])
