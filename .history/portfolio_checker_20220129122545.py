...
async def main():
  ...
  report = report_database.getReports(now, 20)
  ...
  price_data = {}
  for stock in stocks:
    data = api.get_bars(stock, TimeFrame.Day, "2021-10-26", "2021-10-27", adjustment='raw')
  ...
  for entry in report.entries:
    ...
    percents.append([entry, profit, percent])
  
  percents = sorted(percents, key=lambda foo: foo[2], reverse=True)
  for res in percents:
    print(res[0], round(res[1], 2), round(res[2], 3))
  print("Profit", round(sum(percents), 2))
